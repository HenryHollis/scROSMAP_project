...
ggplot(my_rosmap_meta, aes(x = pmi, fill = as.factor(cond_cogdx))) +
  ...
  labs(title = "Overlaid Histograms", x = "PMI", y = "Frequency", fill = "Condition") +
  theme_minimal()
...
write.table(out, "ALL_Inh_FiltByExprDefault_TMM.csv", sep = ',', row.names = F)
...
colnames(design) = c("CTL","AD", "Batch10_64", "pmi", "sex")
...
write.table(qlf$table, "All_Inh/All_Inh_filtByExpr_edgeR_DE_QLFTest.csv", sep =',', row.names = T, col.names = NA )
```
