...
```{r}
#For gene symbol remapping purposes, I want all the genes expressed in my counts, here I get genes from pseudobulk (all cell types):
...
run_fgsea(rnk_files, gene_remapping_dict, pathways, possible_human_gene_names, min_size = 5, max_size = 500)
```

Run fgsea on sleep related pathways from https://doi.org/10.1152/physiolgenomics.00275.2006

```{r}
sleep_wake_pathways = gmtPathways("./MsigDB_gmts_for_GSEA/sleep_wake_pathways_w_human_orthologs.gmt")
...
run_fgsea(rnk_files, gene_remapping_dict, sleep_wake_pathways,possible_human_gene_names, dir = "fGSEA_results_sleep_wake", max_size = 10000) #make max_size big because these pathways of interest are large
```
