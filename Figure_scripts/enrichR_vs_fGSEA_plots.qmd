...
```{r}
plot_fgsea_vs_enrichr = function(enrichR_path, fgsea_path, title,  marker_color = "#000000", libname = "KEGG", fgsea_enrichment = "all", max.overlaps = 10){
  tryCatch({
    ...
    if(fgsea_enrichment == "all"){
      ...
      p = ggplot(plot_df, aes(NES, -log10(`Adjusted p-value`), color = -log10(padj), label = str_to_title(pathway.x)))+
        ...
        ggrepel::geom_text_repel(max.overlaps = max.overlaps, color = "black")+
        xlab("fGSEA NES")+
        ylab("-Log(enrichR BH.q)")+
        ggtitle(str_wrap(title, 20))+
        theme_minimal()+
