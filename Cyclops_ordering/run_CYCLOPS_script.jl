...
:eigen_contr_var => 0.04, 						# Minimum amount of variance required to be captured by a single dimension of eigen gene data
...
:train_max_steps => 2400, 						# Maximum number of training steps per model
...
:train_collection_time_balance => 1,					# How is the true time loss rescaled
...
@everywhere include(joinpath(homedir(), "Box Sync",  "Henry_stuff", "AD_project","CYCLOPS", "CYCLOPS.jl"))

subset_subjects = TMM[:, Vector(TMM[1,:])  .!= "cond_1"]


eigen_data, metricDataframe, correlationDataframe, best_model, options = CYCLOPS.Fit( TMM, seed_genes, training_parameters)
#plot3d(eigen_data[1:3,:]', names(TMM)[Not(1)])
coords = CYCLOPS.CovariatesThroughEncodingDense(eigen_data, best_model)
coords =  mapreduce(permutedims, vcat, coords); using Plots; Plots.scatter(coords[:, 1], coords[:, 2])
...
eigen_data2, metricDataframe2, correlationDataframe2, model2, options2  = CYCLOPS.ReApplyFit(best_model, TMM, subset_subjects, seed_genes, training_parameters)
